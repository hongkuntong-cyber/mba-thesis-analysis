...
| median_sales | 全部观察周销量中位数 | 画像与完整敏感性网格 | 修订 1.2：在非负间歇序列中其零/正状态近似为 ADI≈2 的离散阈值，不参与确认性主特征选择；仍完整报告结果，不改用正销量中位数 |
...
允许的确认性正式组合：ADI、CV²固定，附加特征从 mean_sales、std_sales、nonzero_mean、acf1 中确定性枚举，但 mean_sales 与 nonzero_mean 不共存。

完整敏感性组合另将 median_sales 加回原候选空间并按同一 500 次协议运行；这些结果用于暴露零值质量点的影响，不参与操作候选或论文主特征集选择。修订原因和已观察预检结果见 `protocol/amendment_v1.2.md`。
